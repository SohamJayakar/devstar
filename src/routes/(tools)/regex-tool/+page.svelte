            ...
            <div class="relative ml-4">
                ...
                {#if showDropdown}
                    <div class="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded shadow-lg max-h-40 overflow-y-auto z-10">
                        ...
                        <label class="block p-2 text-gray-900 dark:text-gray-300"><input type="checkbox" bind:checked={$caseInsensitiveFlag} /> case insensitive (i)</label>
                        <label class="block p-2 text-gray-900 dark:text-gray-300"><input type="checkbox" bind:checked={$multilineFlag} /> multiline (m)</label>
                        <label class="block p-2 text-gray-900 dark:text-gray-300"><input type="checkbox" bind:checked={$singleLineFlag} /> single line (s)</label>
                        <label class="block p-2 text-gray-900 dark:text-gray-300"><input type="checkbox" bind:checked={$unicodeFlag} /> unicode (u)</label>
                        <label class="block p-2 text-gray-900 dark:text-gray-300"><input type="checkbox" bind:checked={$stickyFlag} /> sticky (y)</label>
                    </div>
                {/if}
            </div>
        </div>
